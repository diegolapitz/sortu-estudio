...
export default {
  async fetch(request) {
    ...
    if (!payload || typeof payload !== 'object') return json({ error: 'No pudimos procesar la consulta.' }, 400);
    ...
    if (!nombre || !validEmail || !mensaje) {
      return json({ error: 'Completá tu nombre, email y mensaje para continuar.' }, 400);
    }
    if (!process.env.RESEND_API_KEY || !process.env.RESEND_FROM) {
      console.error('Missing Resend environment variables.');
      return json({ error: 'El formulario está temporalmente no disponible. Probá nuevamente más tarde.' }, 503);
    }
    ...
    try {
      ...
      if (!resendResponse.ok) {
        ...
        return json({ error: 'No pudimos enviar el mensaje. Probá nuevamente en unos minutos.' }, 502);
      }
    } catch (error) {
      console.error('Contact form delivery failed.', error instanceof Error ? error.message : 'Unknown error');
      return json({ error: 'No pudimos enviar el mensaje. Revisá tu conexión e intentá otra vez.' }, 502);
    }
